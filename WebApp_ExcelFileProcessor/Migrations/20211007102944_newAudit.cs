using System;
using Microsoft.EntityFrameworkCore.Migrations;

namespace WebApp_ExcelFileProcessor.Migrations
{
    public partial class newAudit : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DeleteData(
                table: "Genders",
                keyColumn: "GenderId",
                keyValue: new Guid("3136d187-d796-419c-a4a7-5bcf0aff2f26"));

            migrationBuilder.DeleteData(
                table: "Genders",
                keyColumn: "GenderId",
                keyValue: new Guid("6d19436a-8eaa-4286-8c9f-4fcec45bc840"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("0cfa05a8-3ecc-4b65-ae32-5ba508f38db3"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("14dd5839-8b79-433c-a31f-468dc5bde314"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("1b8c7043-5ddd-4cd9-b6b2-25700e00b392"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("1d9f8f15-ab1e-4a2b-a4cf-ffdddb63714f"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("269636da-82a3-4dea-9471-064776dc0887"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("2d42d2eb-674f-4033-bed5-4dc2f1044331"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("304e52c3-418d-4457-b949-0db9ed58f89e"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("3765d4ba-7224-40aa-b24c-632bc7eec1dc"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("37a8e15c-5421-4d68-928b-16a80beebb69"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("383d00f5-4044-43a4-844b-7285c135199f"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("39c2b4af-d2ca-47f0-aebe-45285620255d"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("3c15adf8-4b68-46ac-9153-294efbf762ca"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("4a52c312-672f-4f8b-a9bb-93dfd2e37754"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("4b4fee75-db7d-44a3-880b-541a42b45847"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("4ff90ecf-bdc3-40f2-889e-6980b41749d3"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("58429448-6d02-434e-9c5e-ec13f7a6727d"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("59ab3ce0-f63c-42ad-b930-3682144111be"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("5b00e89e-72d1-4b93-b56c-a2210c3a883a"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("5d5e576d-ca28-4362-a087-57b73e67ef5e"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("77322982-b8cd-4270-871a-015022087131"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("80cbc098-4daa-48de-a2ea-4b00fb06b1dc"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("95765801-96d7-43f2-9da2-711858a0d97d"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("95a456fc-c096-4cdf-8d56-0d296de9aa69"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("99fd044f-8041-406e-8104-e228c3031b68"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("a9466098-2a5a-45dc-8434-5c29fd85aad8"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("b32a0ecc-9ddb-4b92-b936-1e9ea584e36e"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("b9f34543-b45c-4212-8239-3f6e8e57cbcd"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("d40b0f3e-227f-4823-8019-128f00409059"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("dcc3c21d-3233-433d-a30a-309ece47635f"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("e479d001-44f8-4114-91b7-28494d9fc4ea"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("e7024db6-c513-4eed-a7cb-a2e0f5ce6b75"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("eee8724c-a65d-408e-8055-28b009d809e2"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("f83199c7-d83c-4658-8c90-51ff47d651db"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("fdd0845f-ba25-4e20-8693-82ac1447de1a"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("feb607bd-041f-4c95-877a-6aece586bfec"));

            migrationBuilder.DeleteData(
                table: "StudentColors",
                keyColumn: "StudentColorId",
                keyValue: new Guid("445eb412-edf6-439a-a42f-9a239bade63f"));

            migrationBuilder.DeleteData(
                table: "StudentColors",
                keyColumn: "StudentColorId",
                keyValue: new Guid("a31c4f91-1af0-47ee-89fe-7630d7e971c0"));

            migrationBuilder.DeleteData(
                table: "StudentColors",
                keyColumn: "StudentColorId",
                keyValue: new Guid("dcd4ffcf-02f8-44be-a9a3-ae32e1aca454"));

            migrationBuilder.DeleteData(
                table: "StudentGroups",
                keyColumn: "StudentGroupId",
                keyValue: new Guid("21bec561-ac0b-4fb9-bac3-81503ef37956"));

            migrationBuilder.DeleteData(
                table: "StudentGroups",
                keyColumn: "StudentGroupId",
                keyValue: new Guid("28d59f1c-eb6d-43aa-bf4c-05b1f9a00205"));

            migrationBuilder.DeleteData(
                table: "StudentGroups",
                keyColumn: "StudentGroupId",
                keyValue: new Guid("74a91b56-52c9-4d97-8a72-0f5d49035215"));

            migrationBuilder.DeleteData(
                table: "StudentGroups",
                keyColumn: "StudentGroupId",
                keyValue: new Guid("9270fde0-2b75-44c8-91cc-b1716dee4e50"));

            migrationBuilder.DeleteData(
                table: "StudentGroups",
                keyColumn: "StudentGroupId",
                keyValue: new Guid("fb580e6a-0a2f-4cf0-a6f5-5eb913630dac"));

            migrationBuilder.CreateTable(
                name: "Audits",
                columns: table => new
                {
                    AuditId = table.Column<Guid>(type: "uniqueidentifier", nullable: false),
                    UserId = table.Column<string>(type: "nvarchar(450)", nullable: true),
                    Type = table.Column<string>(type: "nvarchar(max)", nullable: true),
                    TableName = table.Column<string>(type: "nvarchar(max)", nullable: true),
                    DateTime = table.Column<DateTime>(type: "datetime2", nullable: false),
                    OldValues = table.Column<string>(type: "nvarchar(max)", nullable: true),
                    NewValues = table.Column<string>(type: "nvarchar(max)", nullable: true),
                    AffectedColumns = table.Column<string>(type: "nvarchar(max)", nullable: true),
                    PrimaryKey = table.Column<string>(type: "nvarchar(max)", nullable: true)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_Audits", x => x.AuditId);
                    table.ForeignKey(
                        name: "FK_Audits_AspNetUsers_UserId",
                        column: x => x.UserId,
                        principalTable: "AspNetUsers",
                        principalColumn: "Id",
                        onDelete: ReferentialAction.Restrict);
                });

            migrationBuilder.UpdateData(
                table: "AspNetRoles",
                keyColumn: "Id",
                keyValue: "ba566e0f-3d52-4967-a62c-1965b9f1e251",
                column: "ConcurrencyStamp",
                value: "c3425757-c52d-422e-a042-c8a9e3a08915");

            migrationBuilder.UpdateData(
                table: "AspNetUsers",
                keyColumn: "Id",
                keyValue: "1b824f5d-01b1-411b-8e44-5b05003ef81e",
                columns: new[] { "ConcurrencyStamp", "PasswordHash", "SecurityStamp" },
                values: new object[] { "369fed18-4612-4862-8ee1-dd80a4889a9b", "AQAAAAEAACcQAAAAENvayXYQIApa/YII8jETYQudNUSOrCS/14zTduglEdeOF5K7Gz31j6xKcwUnnruYYA==", "79bbc203-11ed-4d1e-af86-b5f32871a9aa" });

            migrationBuilder.InsertData(
                table: "Genders",
                columns: new[] { "GenderId", "DateCreated", "GenderChar", "GenderName", "IsDeleted" },
                values: new object[,]
                {
                    { new Guid("4a1f43ef-154e-43fb-9cf0-a646e7f4bc4b"), new DateTime(2021, 10, 7, 12, 29, 43, 853, DateTimeKind.Local).AddTicks(5380), "F", "Female", false },
                    { new Guid("e7ee7408-aadc-4741-8ef2-c7952d2087ec"), new DateTime(2021, 10, 7, 12, 29, 43, 855, DateTimeKind.Local).AddTicks(1974), "M", "Male", false }
                });

            migrationBuilder.InsertData(
                table: "StudentClasses",
                columns: new[] { "StudentClassId", "ClassChar", "DateCreated", "DisplayName", "GradeInt", "IsDeleted" },
                values: new object[,]
                {
                    { new Guid("1b6114ab-ef72-4a06-affc-3b39e0c02093"), "B", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2882), "8B", 8, false },
                    { new Guid("c2b5c8bb-3046-4691-a314-b4f43b1c73dc"), "A", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2863), "8A", 8, false },
                    { new Guid("d10be4b5-3f0d-473c-aa68-1db863b0db10"), "C", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2990), "11C", 11, false },
                    { new Guid("3f7f6dd8-cfcf-4bee-9692-ae5f9a23405c"), "D", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2995), "11D", 11, false },
                    { new Guid("f9e1ec52-6773-40bd-b87f-85b6cc24d98c"), "E", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2998), "11E", 11, false },
                    { new Guid("ae0dc0c1-54bb-4251-89e7-6733041d3c19"), "F", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(3002), "11F", 11, false },
                    { new Guid("d8d155d9-6f8b-47fa-84fd-ea472687f331"), "G", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2979), "10G", 10, false },
                    { new Guid("17782ef7-3adb-4af6-ad88-594c349c238b"), "G", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(3010), "11G", 11, false },
                    { new Guid("a1eae9a4-0d81-4e43-b7d3-ad94573f904b"), "B", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(3017), "12B", 12, false },
                    { new Guid("8eeb096d-1ed3-4445-b319-da9717c6b6be"), "C", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(3020), "12C", 12, false },
                    { new Guid("0daa2db2-71f7-4974-8b36-5f9c6e4c5019"), "D", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(3024), "12D", 12, false },
                    { new Guid("6506d253-d139-4eab-8097-ae42223a1d0f"), "E", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(3028), "12E", 12, false },
                    { new Guid("e82d9275-cf52-4b0f-b436-d8a0596fb154"), "F", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(3031), "12F", 12, false },
                    { new Guid("a4238828-e74e-439b-87ea-aaff39275868"), "G", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(3035), "12G", 12, false },
                    { new Guid("93304c37-3588-43ad-9b6b-bcd2f742f5de"), "A", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(3013), "12A", 12, false },
                    { new Guid("95b8cf60-4a41-4f02-b80d-0342ed1b61a8"), "F", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2976), "10F", 10, false },
                    { new Guid("732eb530-c7a1-4ef7-8747-7d8c0da0ad20"), "B", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2987), "11B", 11, false },
                    { new Guid("46b82bff-0bdf-4a4a-bb96-5b0e7fd0e945"), "D", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2966), "10D", 10, false },
                    { new Guid("997371a3-01c6-4385-96a2-1a03e3f2d604"), "C", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2887), "8C", 8, false },
                    { new Guid("cf9a007e-9d09-452d-b083-dffb97c86d3a"), "D", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2910), "8D", 8, false },
                    { new Guid("3c8e1385-3805-45f2-9f99-e9ad04dfebb4"), "E", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2914), "8E", 8, false },
                    { new Guid("d1dd7ad6-9d85-4e62-b77a-4f828553d34d"), "E", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2969), "10E", 10, false },
                    { new Guid("a0295f16-3929-49ab-97fc-d58204de342f"), "G", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2921), "8G", 8, false },
                    { new Guid("5f47ceea-2378-4d0c-819e-a74c4e8b90cc"), "A", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2924), "9A", 9, false },
                    { new Guid("1b146c38-a3ae-417e-9862-e0274c1a534e"), "B", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2928), "9B", 9, false },
                    { new Guid("1a75a9ec-bff8-44e4-b6b6-74e2d89a9571"), "F", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2918), "8F", 8, false },
                    { new Guid("eff0a4ad-af50-43ad-87cc-fb97f634f66b"), "D", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2935), "9D", 9, false },
                    { new Guid("e946f680-2384-42d4-8580-704343afa468"), "C", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2931), "9C", 9, false },
                    { new Guid("1e2050de-e0cc-41a7-a1b2-bdf31605e56e"), "C", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2962), "10C", 10, false },
                    { new Guid("7df3f270-66a7-4f8a-878d-593821ca7240"), "B", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2959), "10B", 10, false },
                    { new Guid("e46bed83-9eda-4442-9978-bfa3a7524edc"), "A", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2983), "11A", 11, false },
                    { new Guid("5a6181fb-2ff0-4a3f-82e5-ada242af2e3e"), "G", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2950), "9G", 9, false },
                    { new Guid("87bdec1e-cc38-4cc1-b5ba-9df51c45dbeb"), "F", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2946), "9F", 9, false },
                    { new Guid("43334717-5c0b-42ee-9a73-cc662af4fe15"), "E", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2942), "9E", 9, false },
                    { new Guid("1ebeaeeb-9b3f-4449-b731-3a8625292003"), "A", new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(2954), "10A", 10, false }
                });

            migrationBuilder.InsertData(
                table: "StudentColors",
                columns: new[] { "StudentColorId", "ColorName", "DateCreated", "IsDeleted" },
                values: new object[,]
                {
                    { new Guid("a8921348-1394-4c4d-af8c-a154f54673ff"), "Gold", new DateTime(2021, 10, 7, 12, 29, 43, 856, DateTimeKind.Local).AddTicks(9491), false },
                    { new Guid("72bd4643-2421-42bd-8622-f46c94f03c1d"), "Unassigned", new DateTime(2021, 10, 7, 12, 29, 43, 856, DateTimeKind.Local).AddTicks(9516), false },
                    { new Guid("0399bc49-934e-4a95-bb08-3cb55abc618c"), "Scarlet", new DateTime(2021, 10, 7, 12, 29, 43, 856, DateTimeKind.Local).AddTicks(9512), false }
                });

            migrationBuilder.InsertData(
                table: "StudentGroups",
                columns: new[] { "StudentGroupId", "DateCreated", "DisplayName", "GroupChar", "GroupInt", "IsDeleted" },
                values: new object[,]
                {
                    { new Guid("bf92baff-c937-4380-999f-962802fe851e"), new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(5340), "G1", "G", 1, false },
                    { new Guid("961485bd-2ae0-4095-ac7a-e989f5d51e0b"), new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(5354), "G2", "G", 2, false },
                    { new Guid("682f07eb-b796-46e6-a598-50179a85e555"), new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(5356), "G3", "G", 3, false },
                    { new Guid("b5b5d4eb-9266-4955-adf2-5d547f042f4b"), new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(5358), "G4", "G", 4, false },
                    { new Guid("c65f63c8-0213-4e95-949f-bd320121a50c"), new DateTime(2021, 10, 7, 12, 29, 43, 857, DateTimeKind.Local).AddTicks(5360), "G5", "G", 5, false }
                });

            migrationBuilder.CreateIndex(
                name: "IX_Audits_UserId",
                table: "Audits",
                column: "UserId");
        }

        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "Audits");

            migrationBuilder.DeleteData(
                table: "Genders",
                keyColumn: "GenderId",
                keyValue: new Guid("4a1f43ef-154e-43fb-9cf0-a646e7f4bc4b"));

            migrationBuilder.DeleteData(
                table: "Genders",
                keyColumn: "GenderId",
                keyValue: new Guid("e7ee7408-aadc-4741-8ef2-c7952d2087ec"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("0daa2db2-71f7-4974-8b36-5f9c6e4c5019"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("17782ef7-3adb-4af6-ad88-594c349c238b"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("1a75a9ec-bff8-44e4-b6b6-74e2d89a9571"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("1b146c38-a3ae-417e-9862-e0274c1a534e"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("1b6114ab-ef72-4a06-affc-3b39e0c02093"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("1e2050de-e0cc-41a7-a1b2-bdf31605e56e"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("1ebeaeeb-9b3f-4449-b731-3a8625292003"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("3c8e1385-3805-45f2-9f99-e9ad04dfebb4"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("3f7f6dd8-cfcf-4bee-9692-ae5f9a23405c"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("43334717-5c0b-42ee-9a73-cc662af4fe15"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("46b82bff-0bdf-4a4a-bb96-5b0e7fd0e945"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("5a6181fb-2ff0-4a3f-82e5-ada242af2e3e"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("5f47ceea-2378-4d0c-819e-a74c4e8b90cc"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("6506d253-d139-4eab-8097-ae42223a1d0f"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("732eb530-c7a1-4ef7-8747-7d8c0da0ad20"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("7df3f270-66a7-4f8a-878d-593821ca7240"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("87bdec1e-cc38-4cc1-b5ba-9df51c45dbeb"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("8eeb096d-1ed3-4445-b319-da9717c6b6be"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("93304c37-3588-43ad-9b6b-bcd2f742f5de"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("95b8cf60-4a41-4f02-b80d-0342ed1b61a8"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("997371a3-01c6-4385-96a2-1a03e3f2d604"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("a0295f16-3929-49ab-97fc-d58204de342f"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("a1eae9a4-0d81-4e43-b7d3-ad94573f904b"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("a4238828-e74e-439b-87ea-aaff39275868"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("ae0dc0c1-54bb-4251-89e7-6733041d3c19"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("c2b5c8bb-3046-4691-a314-b4f43b1c73dc"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("cf9a007e-9d09-452d-b083-dffb97c86d3a"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("d10be4b5-3f0d-473c-aa68-1db863b0db10"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("d1dd7ad6-9d85-4e62-b77a-4f828553d34d"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("d8d155d9-6f8b-47fa-84fd-ea472687f331"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("e46bed83-9eda-4442-9978-bfa3a7524edc"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("e82d9275-cf52-4b0f-b436-d8a0596fb154"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("e946f680-2384-42d4-8580-704343afa468"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("eff0a4ad-af50-43ad-87cc-fb97f634f66b"));

            migrationBuilder.DeleteData(
                table: "StudentClasses",
                keyColumn: "StudentClassId",
                keyValue: new Guid("f9e1ec52-6773-40bd-b87f-85b6cc24d98c"));

            migrationBuilder.DeleteData(
                table: "StudentColors",
                keyColumn: "StudentColorId",
                keyValue: new Guid("0399bc49-934e-4a95-bb08-3cb55abc618c"));

            migrationBuilder.DeleteData(
                table: "StudentColors",
                keyColumn: "StudentColorId",
                keyValue: new Guid("72bd4643-2421-42bd-8622-f46c94f03c1d"));

            migrationBuilder.DeleteData(
                table: "StudentColors",
                keyColumn: "StudentColorId",
                keyValue: new Guid("a8921348-1394-4c4d-af8c-a154f54673ff"));

            migrationBuilder.DeleteData(
                table: "StudentGroups",
                keyColumn: "StudentGroupId",
                keyValue: new Guid("682f07eb-b796-46e6-a598-50179a85e555"));

            migrationBuilder.DeleteData(
                table: "StudentGroups",
                keyColumn: "StudentGroupId",
                keyValue: new Guid("961485bd-2ae0-4095-ac7a-e989f5d51e0b"));

            migrationBuilder.DeleteData(
                table: "StudentGroups",
                keyColumn: "StudentGroupId",
                keyValue: new Guid("b5b5d4eb-9266-4955-adf2-5d547f042f4b"));

            migrationBuilder.DeleteData(
                table: "StudentGroups",
                keyColumn: "StudentGroupId",
                keyValue: new Guid("bf92baff-c937-4380-999f-962802fe851e"));

            migrationBuilder.DeleteData(
                table: "StudentGroups",
                keyColumn: "StudentGroupId",
                keyValue: new Guid("c65f63c8-0213-4e95-949f-bd320121a50c"));

            migrationBuilder.UpdateData(
                table: "AspNetRoles",
                keyColumn: "Id",
                keyValue: "ba566e0f-3d52-4967-a62c-1965b9f1e251",
                column: "ConcurrencyStamp",
                value: "94beac5b-84f7-488e-8197-ba39defc15bc");

            migrationBuilder.UpdateData(
                table: "AspNetUsers",
                keyColumn: "Id",
                keyValue: "1b824f5d-01b1-411b-8e44-5b05003ef81e",
                columns: new[] { "ConcurrencyStamp", "PasswordHash", "SecurityStamp" },
                values: new object[] { "3c516405-8699-4301-b739-fa359489c79c", "AQAAAAEAACcQAAAAECvzTf+C0sq+UIt+HNhjoa5Tzvl1DEwjnqzUKtA8n4a7dvpP2IBXTe6+k+2KhnF2QQ==", "9da82683-a0d2-49df-9496-293578b9114c" });

            migrationBuilder.InsertData(
                table: "Genders",
                columns: new[] { "GenderId", "DateCreated", "GenderChar", "GenderName", "IsDeleted" },
                values: new object[,]
                {
                    { new Guid("6d19436a-8eaa-4286-8c9f-4fcec45bc840"), new DateTime(2021, 10, 7, 12, 20, 56, 173, DateTimeKind.Local).AddTicks(358), "F", "Female", false },
                    { new Guid("3136d187-d796-419c-a4a7-5bcf0aff2f26"), new DateTime(2021, 10, 7, 12, 20, 56, 173, DateTimeKind.Local).AddTicks(8609), "M", "Male", false }
                });

            migrationBuilder.InsertData(
                table: "StudentClasses",
                columns: new[] { "StudentClassId", "ClassChar", "DateCreated", "DisplayName", "GradeInt", "IsDeleted" },
                values: new object[,]
                {
                    { new Guid("269636da-82a3-4dea-9471-064776dc0887"), "B", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9172), "8B", 8, false },
                    { new Guid("1b8c7043-5ddd-4cd9-b6b2-25700e00b392"), "A", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9161), "8A", 8, false },
                    { new Guid("eee8724c-a65d-408e-8055-28b009d809e2"), "C", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9228), "11C", 11, false },
                    { new Guid("304e52c3-418d-4457-b949-0db9ed58f89e"), "D", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9231), "11D", 11, false },
                    { new Guid("1d9f8f15-ab1e-4a2b-a4cf-ffdddb63714f"), "E", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9232), "11E", 11, false },
                    { new Guid("4a52c312-672f-4f8b-a9bb-93dfd2e37754"), "F", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9236), "11F", 11, false },
                    { new Guid("4ff90ecf-bdc3-40f2-889e-6980b41749d3"), "G", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9223), "10G", 10, false },
                    { new Guid("58429448-6d02-434e-9c5e-ec13f7a6727d"), "G", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9238), "11G", 11, false },
                    { new Guid("d40b0f3e-227f-4823-8019-128f00409059"), "B", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9241), "12B", 12, false },
                    { new Guid("e479d001-44f8-4114-91b7-28494d9fc4ea"), "C", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9243), "12C", 12, false },
                    { new Guid("80cbc098-4daa-48de-a2ea-4b00fb06b1dc"), "D", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9245), "12D", 12, false },
                    { new Guid("b32a0ecc-9ddb-4b92-b936-1e9ea584e36e"), "E", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9246), "12E", 12, false },
                    { new Guid("b9f34543-b45c-4212-8239-3f6e8e57cbcd"), "F", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9248), "12F", 12, false },
                    { new Guid("a9466098-2a5a-45dc-8434-5c29fd85aad8"), "G", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9252), "12G", 12, false },
                    { new Guid("2d42d2eb-674f-4033-bed5-4dc2f1044331"), "A", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9239), "12A", 12, false },
                    { new Guid("39c2b4af-d2ca-47f0-aebe-45285620255d"), "F", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9221), "10F", 10, false },
                    { new Guid("fdd0845f-ba25-4e20-8693-82ac1447de1a"), "B", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9226), "11B", 11, false },
                    { new Guid("3765d4ba-7224-40aa-b24c-632bc7eec1dc"), "D", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9216), "10D", 10, false },
                    { new Guid("4b4fee75-db7d-44a3-880b-541a42b45847"), "C", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9185), "8C", 8, false },
                    { new Guid("37a8e15c-5421-4d68-928b-16a80beebb69"), "D", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9187), "8D", 8, false },
                    { new Guid("e7024db6-c513-4eed-a7cb-a2e0f5ce6b75"), "E", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9189), "8E", 8, false },
                    { new Guid("99fd044f-8041-406e-8104-e228c3031b68"), "E", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9220), "10E", 10, false },
                    { new Guid("77322982-b8cd-4270-871a-015022087131"), "G", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9194), "8G", 8, false },
                    { new Guid("3c15adf8-4b68-46ac-9153-294efbf762ca"), "A", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9196), "9A", 9, false },
                    { new Guid("feb607bd-041f-4c95-877a-6aece586bfec"), "B", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9197), "9B", 9, false },
                    { new Guid("dcc3c21d-3233-433d-a30a-309ece47635f"), "F", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9192), "8F", 8, false },
                    { new Guid("95765801-96d7-43f2-9da2-711858a0d97d"), "D", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9203), "9D", 9, false },
                    { new Guid("14dd5839-8b79-433c-a31f-468dc5bde314"), "C", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9200), "9C", 9, false },
                    { new Guid("383d00f5-4044-43a4-844b-7285c135199f"), "C", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9214), "10C", 10, false },
                    { new Guid("5b00e89e-72d1-4b93-b56c-a2210c3a883a"), "B", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9213), "10B", 10, false },
                    { new Guid("f83199c7-d83c-4658-8c90-51ff47d651db"), "A", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9225), "11A", 11, false },
                    { new Guid("0cfa05a8-3ecc-4b65-ae32-5ba508f38db3"), "G", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9209), "9G", 9, false },
                    { new Guid("59ab3ce0-f63c-42ad-b930-3682144111be"), "F", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9208), "9F", 9, false },
                    { new Guid("5d5e576d-ca28-4362-a087-57b73e67ef5e"), "E", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9205), "9E", 9, false },
                    { new Guid("95a456fc-c096-4cdf-8d56-0d296de9aa69"), "A", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(9211), "10A", 10, false }
                });

            migrationBuilder.InsertData(
                table: "StudentColors",
                columns: new[] { "StudentColorId", "ColorName", "DateCreated", "IsDeleted" },
                values: new object[,]
                {
                    { new Guid("445eb412-edf6-439a-a42f-9a239bade63f"), "Gold", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(7787), false },
                    { new Guid("a31c4f91-1af0-47ee-89fe-7630d7e971c0"), "Unassigned", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(7802), false },
                    { new Guid("dcd4ffcf-02f8-44be-a9a3-ae32e1aca454"), "Scarlet", new DateTime(2021, 10, 7, 12, 20, 56, 174, DateTimeKind.Local).AddTicks(7800), false }
                });

            migrationBuilder.InsertData(
                table: "StudentGroups",
                columns: new[] { "StudentGroupId", "DateCreated", "DisplayName", "GroupChar", "GroupInt", "IsDeleted" },
                values: new object[,]
                {
                    { new Guid("fb580e6a-0a2f-4cf0-a6f5-5eb913630dac"), new DateTime(2021, 10, 7, 12, 20, 56, 175, DateTimeKind.Local).AddTicks(1615), "G1", "G", 1, false },
                    { new Guid("21bec561-ac0b-4fb9-bac3-81503ef37956"), new DateTime(2021, 10, 7, 12, 20, 56, 175, DateTimeKind.Local).AddTicks(1628), "G2", "G", 2, false },
                    { new Guid("9270fde0-2b75-44c8-91cc-b1716dee4e50"), new DateTime(2021, 10, 7, 12, 20, 56, 175, DateTimeKind.Local).AddTicks(1631), "G3", "G", 3, false },
                    { new Guid("28d59f1c-eb6d-43aa-bf4c-05b1f9a00205"), new DateTime(2021, 10, 7, 12, 20, 56, 175, DateTimeKind.Local).AddTicks(1633), "G4", "G", 4, false },
                    { new Guid("74a91b56-52c9-4d97-8a72-0f5d49035215"), new DateTime(2021, 10, 7, 12, 20, 56, 175, DateTimeKind.Local).AddTicks(1635), "G5", "G", 5, false }
                });
        }
    }
}
